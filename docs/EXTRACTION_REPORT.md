...
Date 2026-09-25 (v2, after recovery round 2) · plan artifact 8c115402 (5 phases) · scope: 389 included studies, 153,701 samples, 174,022 runs.

## 1. What was produced
* `sample_determinations.parquet` — **527,474 determinations** (one per sample × field) over 150,544 samples in 379 studies; columns follow `catalog_schema.sql::sample_determinations` plus `route` (R1–R4), `scope` (sample|group), `group_audit`. Every row passes `validate_row()` (labelled ≤12-word verbatim quote, controlled vocabularies).
